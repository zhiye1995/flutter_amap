...
public class Animate_CameraActivity extends Activity implements OnClickListener,
	...
	public void onClick(View v) {
		int id = v.getId();
		 //点击停止动画按钮响应事件
		 if (id == R.id.stop_animation) {
			aMap.stopAnimation();
		} else if (id == R.id.Zhongguancun) {//点击“去中关村”按钮响应事件
			changeCamera(
		...
		} else if (id == R.id.Lujiazui) {//点击“去陆家嘴”按钮响应事件
			changeCamera(
					CameraUpdateFactory.newCameraPosition(new CameraPosition(
			...
			aMap.addMarker(new MarkerOptions().position(Constants.SHANGHAI)
					.icon(BitmapDescriptorFactory
							.defaultMarker(BitmapDescriptorFactory.HUE_RED)));
		 } else if (id == R.id.scroll_left) {//点击向左移动按钮响应事件，camera将向左边移动
			changeCamera(CameraUpdateFactory.scrollBy(-SCROLL_BY_PX, 0), null);
		 } else if (id == R.id.scroll_right) {//点击向右移动按钮响应事件，camera将向右边移动
			changeCamera(CameraUpdateFactory.scrollBy(SCROLL_BY_PX, 0), null);
		} else if (id == R.id.scroll_up) {//点击向上移动按钮响应事件，camera将向上边移动
			changeCamera(CameraUpdateFactory.scrollBy(0, -SCROLL_BY_PX), null);
		} else if (id == R.id.scroll_down) {//点击向下移动按钮响应事件，camera将向下边移动
			changeCamera(CameraUpdateFactory.scrollBy(0, SCROLL_BY_PX), null);
		}
	}
