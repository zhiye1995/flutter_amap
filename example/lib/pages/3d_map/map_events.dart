...
class _MapEventsPageState extends State<MapEventsPage> {
  ...
  void _onPointerEvent(PointerEvent event, String phase) {
    ...
    setState(() {
      _touchText = '触摸事件：屏幕位置 ${p.dx.toStringAsFixed(1)} ${p.dy.toStringAsFixed(1)} ($phase)';
    });
  }
                  ...
                  child: Listener(
                    ...
                    child: AMapWidget(
                      ...
                      onCameraChangeStart: (cameraPosition) {
                        _logDebug('onCameraChangeStart', cameraPosition.encode());
                      },
                      onCameraChangeFinish: (cameraPosition) {
                        ...
                        _logDebug('onCameraChangeFinish', cameraPosition.encode());
                      ...
                      onMarkerDragStart: (markerId, position) {
                        _logDebug(
                            'onMarkerDragStart', '$markerId, ${position.encode()}');
                      },
                      onMarkerDrag: (markerId, position) {
                        _logDebug('onMarkerDrag', '$markerId, ${position.encode()}');
                      },
                      onMarkerDragEnd: (markerId, position) {
                        _logDebug(
                            'onMarkerDragEnd', '$markerId, ${position.encode()}');
                      },
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 2),
              Text(
                touchText.isEmpty ? '（在地图区域滑动；部分平台地图为 PlatformView 时可能无回调）' : touchText,
                style: _mono,
              ),
            ],
          ),
