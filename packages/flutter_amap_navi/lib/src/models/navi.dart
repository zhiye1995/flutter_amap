...
class NaviPoint {
  ...
  Object encode() {
    return <Object?>[position.encode(), name, poiId, startAngle];
  }

  static NaviPoint decode(List<Object?> result) {
    return NaviPoint(
      position: NaviPosition.decode(result[0]! as List<Object?>),
...
class NaviConfig {
    ...
    return NaviConfig(
      ...
      wayPoints: at(6) != null
          ? (at(6) as List)
                ...
                .toList()
          : null,
      drivingStrategy: NaviDrivingStrategy.fromId(at(7) as int?),
  ...
  NaviConfig copyWith({
    ...
    NaviDrivingStrategy? drivingStrategy,
    int? travelStrategy,
    bool? multipleRoute,
...
class CruiseCongestionLink {
  CruiseCongestionLink({
    this.status,
    this.coords = const <NaviPosition>[],
    this.raw,
  });
  ...
  /// 拥堵 link 的形状点集合。
  final List<NaviPosition> coords;
      ...
      for (final e in rawCoords) {
        if (e is Map) {
          ...
          if (lat != null && lng != null) {
            coords.add(NaviPosition(latitude: lat, longitude: lng));
          }
        }
      }
    }
