...
///
/// - 支持 **Android** 和 **iOS** 平台。
/// - 调用前请先执行 [AMapNavi.init] 完成 SDK 初始化与隐私合规设置。
...
///     end: NaviPoint(
///       position: NaviPosition(latitude: 39.908, longitude: 116.397),
///       name: "天安门",
///     ),
///   ),
/// );
...
class AMapNavi {
  ...
  /// 初始化导航 SDK。可重复调用，后一次配置会重新应用到原生 SDK。
  static Future<void> init({required NaviSdkConfig config}) async {
    await AMapNaviPlatformInterface.instance.initialize(config);
    if (config.preloadNaviIcons) {
  ...
  static Future<void> startNavigation({required NaviConfig config}) async {
    _ensureInitialized();
    if (isCruising) {
      throw StateError('无法在智能巡航进行中启动导航：请先调用 stopCruiseMode()');
    }
  ...
  static Future<void> stopNavigation() async {
    await AMapNaviPlatformInterface.instance.stopNavigation();
  ...
  static Future<void> startCruise(CruiseConfig config) async {
    _ensureInitialized();
    if (isNavigating) {
      throw StateError('无法在导航进行中开启巡航：请先调用 stopNavigation()');
    }
    // Android 巡航会复用 AMapNaviListener，并可能在原生调用返回前发出
    // startNavi 事件。先进入巡航态，避免该事件把 Dart 状态误置为导航中。
    _setIsCruising(true);
    try {
      await AMapNaviPlatformInterface.instance.startCruiseMode(config);
    } catch (_) {
  ...
  static Future<void> stopCruiseMode() async {
    await AMapNaviPlatformInterface.instance.stopCruiseMode();
    _setIsCruising(false);
  ...
  static Stream<CruiseTrafficFacilityEvent> get onCruiseTrafficFacility =>
      AMapNaviPlatformInterface.instance.onCruiseTrafficFacility;
