...
class AMapFlutterRouteActivity : AmapRouteActivity() {
    ...
    private var routeMapControls: List<View> = emptyList()

    private val routeMapControlNames = listOf(
        "navi_sdk_lbs_route__recalculate",
        "navi_sdk_lbs_route_strategy_btn",
        "navi_sdk_lbs_route_lbs_btn",
        "navi_sdk_autonavi_zoom_and_preview_view"
    )
    ...
    private fun moveRouteMapControls(content: View) {
        val controls = routeMapControls.takeIf {
            it.size == routeMapControlNames.size && it.all(View::isAttachedToWindow)
        } ?: routeMapControlNames.mapNotNull { name ->
            findViewByResourceEntryName(content, name)
        }.takeIf { it.size == routeMapControlNames.size }?.also {
            routeMapControls = it
        } ?: return

        val offset = 15f * resources.displayMetrics.density
        controls.forEach { control ->
            if (control.translationY != -offset) {
                control.translationY = -offset
            }
        }
    }
