...
/// SDK 初始化配置。
class AMapSdkConfig {
  const AMapSdkConfig({required this.apiKey, this.agreePrivacy = true});
...
class AMapWidgetState extends State<AMapWidget> {
    ...
    switch (defaultTargetPlatform) {
      case TargetPlatform.android:
        return AndroidView(
          ...
          creationParams: {"options": initConfig.encode()},
  ...
  void didUpdateWidget(oldWidget) {
    ...
    if (_geolocationControlEnabled(widget) != null &&
      ...
      config.userLocationConfig =
          config.userLocationConfig?.copyWith(
    ...
    if (_showUserLocation(widget) != null &&
      ...
      config.userLocationConfig =
          ...
          UserLocationConfig(showUserLocation: _showUserLocation(widget));
    }
  ...
  void _syncOverlays([AMapWidget? oldWidget]) {
    ...
    _syncPolylines(
      oldWidget?.polylines ?? const <Polyline>{},
      widget.polylines,
    );
    _syncNavigateArrows(
...
Map<String, Marker> _markersById(Set<Marker> markers) {
  return <String, Marker>{for (final marker in markers) marker.id: marker};
}
...
Map<String, Arc> _arcsById(Set<Arc> arcs) {
  return <String, Arc>{for (final arc in arcs) arc.id: arc};
}

Map<String, Polygon> _polygonsById(Set<Polygon> polygons) {
  return <String, Polygon>{for (final polygon in polygons) polygon.id: polygon};
}
