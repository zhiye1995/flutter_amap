...
旧调用中颜色数量不足、索引越界、重复连续点等输入现在会明确报错。请先修正输入，不要依赖 SDK 静默回退。
...
大地线不能与多彩、渐变、纹理组合；纹理不能与多彩、渐变、虚线组合，避免不同平台静默忽略设置。只允许一个启用的纹理来源。宽度必须为正且有限，坐标必须有效，相邻点不得相同。Arc 还拒绝三点重合或投影后共线。
